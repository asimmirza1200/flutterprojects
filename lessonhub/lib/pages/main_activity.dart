...
class _MainActivityState extends State<MainActivity> {
  ...
  void requeryUnits() async {
    if (this.widget.arguments["courseData"] == null &&
        this.widget.arguments["class"] != null) {
      var res = await Unit.getData(this.widget.arguments["class"].subject.id);
  ...
  Widget getActiveScreen(String activeScreen) {
    switch (activeScreen) {
      case "puzzle":
        return QuizActivity(
          quizzes: this.widget.arguments["courseData"] ?? units,
        );
      case "video":
        return VideoActivity(
          videos: this.widget.arguments["courseData"] ?? units,
        );
      case "book":
        return BookActivity(
          books: this.widget.arguments["courseData"] ?? units,
        );
      case "play":
        return PlayActivity(
          plays: this.widget.arguments["courseData"] ?? units,
        );
    }
  }
  ...
  Widget build(BuildContext context) {
    print("PAIN");
    print(this.widget.arguments["courseData"].runtimeType);
                ...
                child: Container(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 19.0,
                      color:
                          activeScreen == "book" ? Colors.black : Colors.white,
                    ),
                  ),
                  decoration: BoxDecoration(
                    color: activeScreen == "book"
                        ? Colors.white
                        : AppConstants.kBlueColor,
                    border: Border.all(
                ...
                child: Container(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 19.0,
                      color:
                          activeScreen == "play" ? Colors.black : Colors.white,
                    ),
                  ),
                  decoration: BoxDecoration(
                    color: activeScreen == "play"
                        ? Colors.white
                        : AppConstants.kBlueColor,
                    border: Border.all(color: AppConstants.kBlueColor),
                    borderRadius: BorderRadius.only(
